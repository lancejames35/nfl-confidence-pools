...
const { getNFLSeasonYear } = require('./getCurrentWeek');

function getCurrentNFLWeek() {
    const now = new Date();
    const year = getNFLSeasonYear(); // Use NFL season year, not calendar year
...
function getWeekDeadline(week, year = getNFLSeasonYear()) {
...
function arePicksLocked(week, year = getNFLSeasonYear()) {
    const deadline = getWeekDeadline(week, year);
    return new Date() >= deadline;
}

// Get hours until deadline
function getHoursUntilDeadline(week, year = getNFLSeasonYear()) {
    const deadline = getWeekDeadline(week, year);
    const now = new Date();
